...
[root@localhost knex]# cat    clientMasterSessionChained.js
...
(async (teacherEmail, teacherAutoIncr, clientMasterEmail, layoutName ) => {
        let insertObj = {       'teacherEmail'          :       teacherEmail
                        ...
                        ;
        let clientMasterAutoIncr;
        const val  = await returnClientMasterAutoIncr(insertObj);
        if  ( typeof val == 'object' && val.length > 0 )                {
                clientMasterAutoIncr = val[0].clientMasterAutoIncr;
        } else {
                console.warn("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
                console.table(insertObj);
                console.warn("BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB");
                clientMasterAutoIncr = await returnClientMasterInsert(insertObj);
                console.warn("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ");
        }
        console.log('!!!!!!!!!!!!!!  ending with:', clientMasterAutoIncr);
})( parmTeacherEmail, parmTeacherAutoIncr, parmClientMasterEmail, parmLayoutName, parmSessionName );


function returnClientMasterAutoIncr(insertObj)  {
...
function returnClientMasterInsert(insertObj)    {
                console.warn("CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC");
        console.table(insertObj);
        return knex.from('clientMaster')
                .insert(insertObj)
                .then( val => { return val; } )
...
/*
const async returnClientMasterAutoIncr  = (parmObject) => {
        const returnedVal       =       await returnClientMasterAutoIncr(parmObject)    ;
